...
(function () {
  ...
  // Geometry overview:
  //   - Brand DNA nucleus at canvas center; 4 core nodes orbit it.
  //   - Each core's sub-nodes radiate outward on individual angular spokes,
  //     evenly spaced across a 180° arc centered on the "outward" direction
  //     (the direction from nucleus to core).
  //   - Each spoke runs from the core's CENTER outward at its angle.
  //     Because each spoke exits the hex at a different angle, each
  //     sub-bond hits the hex perimeter at a unique point — no two share
  //     an attachment. The bond line is drawn from center to sub-node,
  //     but the inner portion is covered by the hex fill so visually it
  //     appears to start at the perimeter.
  //   - Radial spokes can never cross each other (they only intersect at
  //     the core center, which is hidden), so the layout is automatically
  //     non-crossing regardless of how many sub-nodes a core has.
  //   - Adding/removing sub-nodes redistributes the angles automatically.
  var MOLECULE = {
    nucleus: { cx: 650, cy: 510 },
    cores: {
      execution:   { cx: 360, cy: 240,
                     subs: ['CONTENT', 'PAID MEDIA', 'SEO/GEO', 'EMAIL', 'SOCIAL', 'AI'] },
      goals:       { cx: 940, cy: 240,
                     subs: ['REVENUE', 'MARKET\nSHARE', 'ACQUISITION', 'RETENTION', 'GROWTH', 'AUTHORITY'] },
      strategies:  { cx: 360, cy: 780,
                     subs: ['CHANNELS', 'TARGETING', 'POSITIONING', 'CONTENT\nPLAN', 'PRICING', 'PRODUCT/\nSERVICE'] },
      environment: { cx: 940, cy: 780,
  ...
  function subLayout(core, idx, count, nucleus) {
    ...
    for (var k = 0; k < count; k++) {
      var t = count > 1 ? k / (count - 1) : 0.5;
      var angle = (outwardAngle - arcSpan / 2) + t * arcSpan;
      positions.push({
        angle: angle,
        x: core.cx + MOLECULE.subRadius * Math.cos(angle),
        y: core.cy + MOLECULE.subRadius * Math.sin(angle)
      });
    }
  ...
  // Sub-node font is 14pt bold DM Mono (set in CSS). For long single-word
  // labels, we tighten the letter-spacing so the label fits inside the hex.
  // For two-line labels, we use dy offsets calibrated to the 14pt size.
  function subLabelTspans(label, cx, cy) {
    if (label.indexOf('\n') === -1) {
      // single-line label; tighten tracking for extra-long single words
      var tightWord = (label.length >= 11);
      var fontAttr = tightWord ? ' style="letter-spacing:.02em"' : '';
      return '<tspan x="' + cx + '" dy="0"' + fontAttr + '>' + esc(label) + '</tspan>';
    }
    // two-line label
    var parts = label.split('\n');
    return '<tspan x="' + cx + '" dy="-7">' + esc(parts[0]) + '</tspan>' +
           '<tspan x="' + cx + '" dy="17">' + esc(parts[1]) + '</tspan>';
  }
  ...
  function moleculeSVG() {
    ...
    // The inner portion of each line is hidden behind the core hex fill,
    // so visually the bond appears to emerge from the hex perimeter at a
    // unique angle per sub-node.
    ...
    Object.keys(m.cores).forEach(function (key) {
      ...
      for (var i = 0; i < core.subs.length; i++) {
        ...
        parts.push('<line x1="' + sl.bondStart.x + '" y1="' + sl.bondStart.y + '" x2="' + sl.center.x.toFixed(1) + '" y2="' + sl.center.y.toFixed(1) + '" class="sub-bond" data-parent="' + key + '"/>');
      }
    });
    parts.push('</g>');
